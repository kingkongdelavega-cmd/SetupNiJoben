...
async function deductStockInMemory({ itemId, quantity, reason, notes }) {
  ...
  // Ensure numeric subtraction (prevents accidental string math)
  ...
  // Replace the slot by index instead of mutating the found object in place.
  // This guarantees the update is visible to every other reference to the
  // `inventory` array, even if the original item object turns out to be
  // frozen/immutable (e.g. shared seed fixtures used across test files).
  inventory[idx] = updatedItem

  // Keep compatibility with other stores' signature.
  void reason
  void notes
...
async function deductStockInSupabase({ itemId, quantity, reason, notes }) {
  ...
  // In unit tests, inventorySupabaseStore.getInventoryById is mocked with:
  // supabase.from(...).select(...)
  // (no .eq chain). So use that store to read current stock.
